...
### subset to burned in graves fire
graves_ft <- burned_ft %>%
  filter(frst_fr_d == "WA4863311833720150814")

### graves ft as factor
graves_ft$ft_id <- as.factor(graves_ft$ft_id)

### subset ft
graves_ft <- graves_ft %>%
  filter(ft_id %in% c("138363", "139872"))

### snap
graves_snap <- st_snap(graves_ft, graves_ft, tolerance = 13)

### combine
graves_unioned <- st_union(graves_snap)

### plot
ggplot() +
  geom_sf(data = graves_unioned, fill = NA, color = "blue") +
  theme_void()

### intersection
graves_unioned <- st_intersection(graves_unioned, wa_fire)

### plot
ggplot() +
  geom_sf(data = graves_unioned, fill = "gray", color = NA) +
  geom_sf(data = wa_fire, fill = NA, color = "black", lwd = 1) +
  theme_void()
ggsave("C:/Users/kjsie/Documents/scoping/graves_fire_diagram.png")

# ### drop geom from object
# st_geometry(wa_fire) <- NULL

### add column with full file path to the fire names
sample_fire_wa <- wa_fire %>% 
...
ggplot() +
  ...
  geom_sf(data = graves_unioned, fill = NA, color = "black", lwd = 1,
          linetype = 4) +
  geom_sf(data = wa_fire, fill = NA, color = "black", lwd = 1) +
  theme_void()
...
ggplot() +
  ...
  geom_sf(data = fires_hfr,
          color = "lightblue",
          fill = "lightblue") +
  geom_sf(data = wa_fire,
          color = "red3",
          fill = "red3") +
  theme_void()

### write out
st_write(wa_fire, "C:/Users/kjsie/Documents/scoping/graves_fire_perim.shp")
st_write(fires_hfr, "C:/Users/kjsie/Documents/scoping/all_ft_fires_perim.shp")
```
...
mtbs_path <- mtbs_path[mtbs_path %in% sample_fire_wa$file_name]
...
## write out
st_write(cbi_full,
         "C:/Users/kjsie/Documents/scoping/graves_pts.shp")
