...
### function to get dist for each pt
get_dist_df <- function(i) {
  ...
  ### deal with NAs
  if (nrow(line_i) == 0) {
    return(data.frame(uid = pt_i$uid, 
                      dist_bound = NA_real_))
  }
  ...
  ### make df
  df_i <- data.frame(uid = pt_i$uid,
                     dist_bound = as.numeric(dist_i))
...
### run in parallel
dist_list <- future_map(1:nrow(pts), 
                        get_dist_df,
                        .progress = TRUE,
                        .options = furrr_options(seed = TRUE))
...
### combine with pt attributes
pt_att <- pts %>%
  dplyr::select(uid, fire_id)
st_geometry(pt_att) <- NULL
dist_df <- merge(x = dist_df,
                 y = pt_att,
                 by = "uid",
                 all = TRUE)
```
